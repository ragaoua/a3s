"""Shared A2A client helpers for the test suites.

A2A 1.0 types are protobuf messages, so reading them means `HasField` checks and
`MessageToDict` conversions rather than pydantic attribute access. These helpers
keep that plumbing — and the "resolve the card, build a client, drive one call"
boilerplate every suite needs — in one place.
"""
